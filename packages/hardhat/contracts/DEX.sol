...
contract DEX {
     ...
     */
    event EthToTokenSwap(address sender, uint256 eth, uint256 balloons);

    /**
     * @notice Emitted when tokenToEth() swap transacted
     */
    event TokenToEthSwap(address sender, uint256 eth, uint256 balloons);
     ...
     */
    function ethToToken() public payable returns (uint256 tokens) {
        require(msg.value > 0, "ETH Required");
        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokenOutput = price(msg.value, ethReserve, token_reserve);

        require(token.transfer(msg.sender, tokenOutput), "swap failed");
